...
export class Base {
    ...
    createControlTower(x, y, z) {
        ...
        deck.position.y = 14; towerGroup.add(deck);
        const roof = new THREE.Mesh(new THREE.BoxGeometry(8.2, 0.5, 8.2), this.concreteMat); roof.position.y = 16; towerGroup.add(roof);
        const body = new CANNON.Body({ mass: 0 }); body.addShape(new CANNON.Box(new CANNON.Vec3(3, 6, 3)), new CANNON.Vec3(0, 6, 0));
    ...
    createHangar(x, y, z) {
        ...
        hangarGeo.rotateZ(Math.PI / 2); hangarGeo.rotateX(Math.PI / 2);
        const hangar = new THREE.Mesh(hangarGeo, this.corrugatedMat); hangarGroup.add(hangar);
        const body = new CANNON.Body({ mass: 0 });
        for (let i = 0; i < 5; i++) {
            const angle = (i / 4) * Math.PI; const px = Math.cos(angle) * 9.5; const py = Math.sin(angle) * 9.5;
            const box = new CANNON.Box(new CANNON.Vec3(1.5, 0.5, 15)); const quat = new CANNON.Quaternion();
            quat.setFromEuler(0, 0, angle + Math.PI/2); body.addShape(box, new CANNON.Vec3(px, py, 0), quat);
        }
        body.position.set(this.position.x + x, this.position.y, this.position.z + z); this.world.addBody(body);
    }
    createCantonment() {
        ...
        for(let row = 0; row < 2; row++) {
            const side = row === 0 ? -1 : 1; const xOffset = side * (streetWidth/2 + hutSpacingX/2);
            for(let i = 0; i < 3; i++) {
                const zOffset = -25 + (i * hutSpacingZ); this.createQuonsetHut(xOffset - 40, 0, zOffset);
            }
    ...
    createQuonsetHut(x, y, z) {
        ...
        hutGeo.rotateZ(Math.PI / 2); hutGeo.rotateX(Math.PI / 2);
        const hut = new THREE.Mesh(hutGeo, this.corrugatedMat); hutGroup.add(hut);
        const wallGeo = new THREE.CircleGeometry(radius, 24, 0, Math.PI);
        const wallF = new THREE.Mesh(wallGeo, this.woodMat); wallF.position.set(0, 0, -length/2); hutGroup.add(wallF);
        const wallB = wallF.clone(); wallB.position.z = length/2; wallB.rotation.y = Math.PI; hutGroup.add(wallB);
        const door = new THREE.Mesh(new THREE.PlaneGeometry(1, 2), this.blackMat); door.position.set(0, 1, -length/2 - 0.01); hutGroup.add(door);
        const step = new THREE.Mesh(new THREE.BoxGeometry(2, 0.3, 1), this.concreteMat); step.position.set(0, 0.15, -length/2 - 0.5); hutGroup.add(step);
        const body = new CANNON.Body({ mass: 0 });
        for(let i=0; i<3; i++) {
            const angle = (i/2) * Math.PI; const px = Math.cos(angle) * 3; const py = Math.sin(angle) * 3;
            const box = new CANNON.Box(new CANNON.Vec3(1, 0.5, length/2)); const quat = new CANNON.Quaternion();
            quat.setFromEuler(0, 0, angle + Math.PI/2); if(py > 0) body.addShape(box, new CANNON.Vec3(px, py, 0), quat);
        }
    ...
    createWashhouse(x, y, z) {
        ...
        const w = 5, l = 8, h = 3;
        const slab = new THREE.Mesh(new THREE.BoxGeometry(w + 1, 0.2, l + 1), this.concreteMat); slab.position.y = 0.1; washGroup.add(slab);
        const building = new THREE.Mesh(new THREE.BoxGeometry(w, h, l), this.creamSidingMat); building.position.y = h/2 + 0.2; washGroup.add(building);
        const roof = new THREE.Mesh(new THREE.BoxGeometry(w + 0.5, 0.2, l + 0.5), this.darkGreenMat); roof.position.y = h + 0.3; roof.rotation.x = 0.1; washGroup.add(roof);
        const body = new CANNON.Body({ mass: 0 });
    ...
    createFortifications() {
        ...
        bounds.forEach(b => {
            if(b.isSouth) {
                ...
                this.createDoubleFence(-gateOpening/2 - sideLen/2, b.z, sideLen, b.rot);
            } else { this.createDoubleFence(b.x, b.z, b.len, b.rot); }
        });
    ...
    createWatchTower(x, y, z) {
        const towerGroup = new THREE.Group(); towerGroup.position.set(x, y, z); this.group.add(towerGroup);
        const h = 14; 
        const tower = new THREE.Mesh(new THREE.BoxGeometry(4, h, 4), this.woodMat); tower.position.y = h/2; towerGroup.add(tower);
        const box = new THREE.Mesh(new THREE.BoxGeometry(5, 3.5, 5), this.woodMat); box.position.y = h + 1.75; towerGroup.add(box);
        const roof = new THREE.Mesh(new THREE.BoxGeometry(6, 0.5, 6), this.darkGreenMat); roof.position.y = h + 3.5; towerGroup.add(roof);
        const lightBase = new THREE.Group(); lightBase.position.set(0, h + 3.8, 0); towerGroup.add(lightBase);
        const slMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.8, 0.8, 1.2, 12).rotateX(Math.PI/2), this.metalMat); lightBase.add(slMesh);
        ...
        lightBase.add(beam); this.searchlights.push(lightBase);
        ...
        for(let a = -Math.PI/2; a <= Math.PI/2; a += Math.PI/8) {
            const bx = Math.cos(a) * 4.5; const bz = Math.sin(a) * 4.5;
            for(let h_stack=0; h_stack<3; h_stack++) {
                const bag = new THREE.Mesh(bagGeo, this.sandbagMat); bag.position.set(bx, h_stack*0.4 + 0.25, bz);
                bag.rotation.y = a + Math.PI/2; towerGroup.add(bag);
                const bagShape = new CANNON.Cylinder(0.25, 0.25, 1.0, 8); const bagQuat = new CANNON.Quaternion();
                bagQuat.setFromEuler(0, a + Math.PI/2, Math.PI/2); body.addShape(bagShape, new CANNON.Vec3(bx, h_stack*0.4 + 0.25, bz), bagQuat);
            }
        }
        body.position.set(this.position.x + x, this.position.y, this.position.z + z); this.world.addBody(body);
    }
    createDoubleFence(x, z, length, rotation) {
        ...
        path.rotation.x = -Math.PI/2; path.position.y = 0.05; fenceGroup.add(path);
        const body = new CANNON.Body({ mass: 0 });
        for(let side = -1; side <= 1; side += 2) {
            const sideZ = side * (fenceDist / 2); const postCount = Math.floor(length / 3);
            for(let i=0; i<=postCount; i++) {
                const lx = (i / postCount - 0.5) * length;
                const post = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 3), this.metalMat); post.position.set(lx, 1.5, sideZ); fenceGroup.add(post);
                const arm = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.8, 0.05), this.metalMat);
                arm.position.set(lx, 2.8, sideZ + (side * 0.2)); arm.rotation.x = side * (Math.PI / 4); fenceGroup.add(arm);
            }
            ...
            mesh.position.set(0, 1.25, sideZ); fenceGroup.add(mesh);
            ...
            for(let i=0; i<coilCount; i++) {
                ...
                coil.position.set(lx, 0.4, sideZ); coil.rotation.y = Math.PI/2; fenceGroup.add(coil);
            }
    ...
    createMainGate(x, z) {
        ...
        const signPost = new THREE.Mesh(new THREE.BoxGeometry(0.15, 3, 0.15), this.woodMat); signPost.position.y = 1.5; signGroup.add(signPost);
        const signBoard = new THREE.Mesh(new THREE.BoxGeometry(3, 1.5, 0.1), this.whiteWoodMat); signBoard.position.y = 2.5; signGroup.add(signBoard);
        const canvas = document.createElement('canvas'); canvas.width = 512; canvas.height = 256; const ctx = canvas.getContext('2d');
        ctx.fillStyle = 'white'; ctx.fillRect(0,0,512,256); ctx.fillStyle = 'black'; ctx.font = 'bold 80px Courier New'; ctx.textAlign = 'center';
    ...
    createHeadquarters(x, y, z) {
        ...
        const w = 12, l = 15, h = 7;
        const main = new THREE.Mesh(new THREE.BoxGeometry(w, h, l), this.whiteWoodMat); main.position.y = h/2; main.castShadow = true; hqGroup.add(main);
        const amGeo = new THREE.BoxGeometry(w + 0.4, 0.1, 0.5);
        for(let i=0; i<2; i++) {
            const am = new THREE.Mesh(amGeo, this.darkGreenMat); am.position.set(0, 2.5 + i*3, -l/2 - 0.2); am.rotation.x = -0.3; hqGroup.add(am);
        }
        const porch = new THREE.Mesh(new THREE.BoxGeometry(4, 0.4, 2), this.concreteMat); porch.position.set(0, 0.2, -l/2 - 1); hqGroup.add(porch);
        const colGeo = new THREE.BoxGeometry(0.3, h, 0.3);
        const colL = new THREE.Mesh(colGeo, this.whiteWoodMat); colL.position.set(-1.8, h/2, -l/2 - 1.8); hqGroup.add(colL);
        const colR = colL.clone(); colR.position.x = 1.8; hqGroup.add(colR);
        const portRoof = new THREE.Mesh(new THREE.BoxGeometry(4.4, 0.3, 2.4), this.darkGreenMat); portRoof.position.set(0, h, -l/2 - 1); hqGroup.add(portRoof);
        const flagGroup = new THREE.Group(); flagGroup.position.set(0, 0, -l/2 - 8); hqGroup.add(flagGroup);
        const court = new THREE.Mesh(new THREE.CylinderGeometry(4, 4, 0.2, 32), this.stoneMat); flagGroup.add(court);
        const mast = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.2, 15), this.whiteWoodMat); mast.position.y = 7.5; flagGroup.add(mast);
        const crosstree = new THREE.Mesh(new THREE.BoxGeometry(3, 0.1, 0.1), this.whiteWoodMat); crosstree.position.y = 12; flagGroup.add(crosstree);
        const finial = new THREE.Mesh(new THREE.SphereGeometry(0.15), new THREE.MeshStandardMaterial({color: 0xffd700, metalness: 1}));
        finial.position.y = 15; flagGroup.add(finial);
        const sign = new THREE.Mesh(new THREE.PlaneGeometry(2, 0.8), this.whiteWoodMat); sign.position.set(0, 1.5, -l/2 - 1.96); hqGroup.add(sign);
        const body = new CANNON.Body({ mass: 0 }); body.addShape(new CANNON.Box(new CANNON.Vec3(w/2, h/2, l/2)), new CANNON.Vec3(0, h/2, 0));
    ...
    createParadeGround(x, y, z) {
        ...
        const w = 40, l = 40;
        const ground = new THREE.Mesh(new THREE.PlaneGeometry(w, l), this.dirtMat); ground.rotation.x = -Math.PI/2; ground.position.y = 0.05; pgGroup.add(ground);
        const stoneGeo = new THREE.BoxGeometry(0.6, 0.3, 0.4);
        for(let side = 0; side < 4; side++) {
            const isHorizontal = side % 2 === 0; const length = isHorizontal ? w : l; const count = Math.floor(length / 1.5);
            for(let i=0; i<count; i++) {
                const s = new THREE.Mesh(stoneGeo, this.stoneMat); const pos = (i / count - 0.5) * length; const offset = 20.2;
                if(side === 0) s.position.set(pos, 0.15, -offset); if(side === 1) s.position.set(offset, 0.15, pos);
                if(side === 2) s.position.set(pos, 0.15, offset); if(side === 3) s.position.set(-offset, 0.15, pos);
                s.rotation.y = isHorizontal ? 0 : Math.PI/2; pgGroup.add(s);
            }
        }
    }
    ...
    createMotorPoolComplex(x, y, z) {
        ...
        const floor = new THREE.Mesh(new THREE.BoxGeometry(shedW, 0.2, shedL), this.oilStainedConcreteMat); floor.position.y = 0.1; mpGroup.add(floor);
        const backWall = new THREE.Mesh(new THREE.BoxGeometry(shedW, shedH, 0.5), this.woodMat); backWall.position.set(0, shedH/2, shedL/2 - 0.25); mpGroup.add(backWall);
        const sideWallGeo = new THREE.BoxGeometry(0.5, shedH, shedL);
        const wallL = new THREE.Mesh(sideWallGeo, this.woodMat); wallL.position.set(-shedW/2 + 0.25, shedH/2, 0); mpGroup.add(wallL);
        const wallR = wallL.clone(); wallR.position.x = shedW/2 - 0.25; mpGroup.add(wallR);
        const roof = new THREE.Mesh(new THREE.BoxGeometry(shedW + 1, 0.2, shedL + 1), this.corrugatedMat); roof.position.y = shedH; roof.rotation.x = -0.1; mpGroup.add(roof);
        const pit = new THREE.Mesh(new THREE.BoxGeometry(3, 0.1, 8), this.blackMat); pit.position.set(0, 0.16, -1); mpGroup.add(pit);
        const body = new CANNON.Body({ mass: 0 });
    ...
    createFuelFarm(x, y, z) {
        ...
        const breakMesh = new THREE.Mesh(new THREE.PlaneGeometry(farmW + 5, farmL + 5), this.dirtMat); breakMesh.rotation.x = -Math.PI/2; breakMesh.position.y = 0.05; fuelGroup.add(breakMesh);
        const drumGeo = new THREE.CylinderGeometry(0.4, 0.4, 1.2, 12); const palletGeo = new THREE.BoxGeometry(2, 0.15, 2);
        const body = new CANNON.Body({ mass: 0 });
        const createStack = (sx, sz) => {
            const pallet = new THREE.Mesh(palletGeo, this.woodMat); pallet.position.set(sx, 0.07, sz); fuelGroup.add(pallet);
            for(let dx = -0.5; dx <= 0.5; dx += 1) {
                for(let dz = -0.5; dz <= 0.5; dz += 1) {
                    for(let dh = 0; dh < 2; dh++) {
                        const drum = new THREE.Mesh(drumGeo, this.metalMat); drum.position.set(sx + dx, 0.75 + (dh * 1.2), sz + dz); fuelGroup.add(drum);
                    }
                }
            }
            body.addShape(new CANNON.Box(new CANNON.Vec3(1, 1.2, 1)), new CANNON.Vec3(sx, 1.2, sz));
        };
        createStack(-3, -2); createStack(3, 3);
        const netH = 3.5; const netGroup = new THREE.Group(); netGroup.position.y = netH; fuelGroup.add(netGroup);
        const netGeo = new THREE.PlaneGeometry(farmW, farmL, 8, 8); const net = new THREE.Mesh(netGeo, this.wireMat);
    ...
    createAmmunitionMags(x, y, z) {
        ...
        bermGeo.rotateZ(Math.PI / 2); bermGeo.rotateX(Math.PI / 2);
        const berm = new THREE.Mesh(bermGeo, this.earthBermMat); magGroup.add(berm);
        const headwall = new THREE.Mesh(new THREE.BoxGeometry(w + 2, h + 1, 0.6), this.formworkConcreteMat);
        headwall.position.set(0, (h+1)/2, l/2); magGroup.add(headwall);
        const wingGeo = new THREE.BoxGeometry(0.6, h + 1, 4);
        const wingL = new THREE.Mesh(wingGeo, this.formworkConcreteMat); wingL.position.set(-w/2 - 1, (h+1)/2, l/2 + 2); wingL.rotation.y = -0.3; magGroup.add(wingL);
        const wingR = wingL.clone(); wingR.position.x = w/2 + 1; wingR.rotation.y = 0.3; magGroup.add(wingR);
        for(let side = -1; side <= 1; side += 2) {
            const door = new THREE.Mesh(new THREE.BoxGeometry(2.1, 3.2, 0.2), this.blastDoorMat); door.position.set(side * 1.1, 1.6, l/2 + 0.3); magGroup.add(door);
        }
    ...
    createTrainingGround(x, y, z) {
        ...
        const wallGroup = new THREE.Group(); wallGroup.position.set(-10, 0, 0); tgGroup.add(wallGroup);
        const wallMain = new THREE.Mesh(new THREE.BoxGeometry(wallW, wallH, 0.4), this.weatheredWoodMat); wallMain.position.y = wallH/2; wallGroup.add(wallMain);
        const wallBody = new CANNON.Body({ mass: 0, shape: new CANNON.Box(new CANNON.Vec3(wallW/2, wallH/2, 0.2)) });
        wallBody.position.set(this.position.x + x - 10, this.position.y + wallH/2, this.position.z + z); this.world.addBody(wallBody);
        const pitW = 6, pitL = 15;
        const sand = new THREE.Mesh(new THREE.PlaneGeometry(pitW, pitL), this.dirtMat); sand.rotation.x = -Math.PI/2; sand.position.y = 0.05; tgGroup.add(sand);
        const logGroup = new THREE.Group(); logGroup.position.set(10, 0, 0); tgGroup.add(logGroup);
        for(let i=0; i<3; i++) {
            ...
            logBody.quaternion.setFromEuler(Math.PI/2, 0, 0); logBody.position.set(this.position.x + x + 10 + log.position.x, this.position.y + 0.4, this.position.z + z + log.position.z);
    ...
    createWaterTower(x, y, z) {
        ...
        const legGeo = new THREE.CylinderGeometry(0.15, 0.2, height, 8); const spread = 4;
        const legPositions = [[spread, spread], [-spread, spread], [spread, -spread], [-spread, -spread]];
        legPositions.forEach(pos => {
            const leg = new THREE.Mesh(legGeo, this.armyGrayMat); leg.position.set(pos[0], height/2, pos[1]);
            leg.rotation.z = -Math.sign(pos[0]) * 0.1; leg.rotation.x = Math.sign(pos[1]) * 0.1; wtGroup.add(leg);
        });
        const jacket = new THREE.Mesh(new THREE.BoxGeometry(1.2, height, 1.2), this.woodMat); jacket.position.y = height/2; wtGroup.add(jacket);
        const tankBase = new THREE.Group(); tankBase.position.y = height; wtGroup.add(tankBase);
        const bottom = new THREE.Mesh(new THREE.SphereGeometry(tankRadius, 16, 16, 0, Math.PI*2, Math.PI/2, Math.PI/2), this.armyGrayMat); tankBase.add(bottom);
        const cylinder = new THREE.Mesh(new THREE.CylinderGeometry(tankRadius, tankRadius, tankHeight, 16), this.armyGrayMat); cylinder.position.y = tankHeight/2; tankBase.add(cylinder);
        const roof = new THREE.Mesh(new THREE.ConeGeometry(tankRadius + 0.2, 2.5, 16), this.metalMat); roof.position.y = tankHeight + 1.25; tankBase.add(roof);
        const body = new CANNON.Body({ mass: 0 }); body.addShape(new CANNON.Cylinder(tankRadius, tankRadius, height + tankHeight, 8));
        body.position.set(this.position.x + x, this.position.y + (height+tankHeight)/2, this.position.z + z); this.world.addBody(body);
    }
}